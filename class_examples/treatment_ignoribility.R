...
W <- runif(bigN,0,5) #uniform between 0-5

X = as.integer(W + 1) #rounds everything down I think
...
e0 <- e[,c(1)] #indexes first column from e
mean(e0)

e1 <- e[,c(2)]  #indexes second column from e
mean(e1)
...
mean(D) #slightly more get the treatment


# USE SUTVA TO MAP POTENTIAL OUTCOMES INTO OBSERVED OUTCOMES
Y = D*Y1 + (1-D)*Y0 #if D = 0, will only be Y0. If D = 1, will only be Y1.
...
ols <- lm(formula = Y ~ D, data=TIA_DATA) #biased estimate for the average treatment effect 
