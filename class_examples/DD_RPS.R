...
# IMPORT CSV DATA 
RPS <- read.csv("/Users/lewiswhite/MEDS/policy_eval/data/RPS_data.csv")
...
RPS%>%
  ...
  dplyr::select(state_name, year, rps_D, rps_ever_adopter, rps_implementation_year)%>%
  View
...
# DD REGRESSION, Y = Wind+Solar installed capacity (MW), using plm package (panel lm)
